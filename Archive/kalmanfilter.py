...
def kalman(curr_pos, curr_meas, curr_input, curr_cov=P_0):
    ...
    # Predict Step
    x_k_p = np.dot(F, x_km1_u) + np.dot(B, u_k)
    P_k_p = np.dot(F, np.dot(P_km1_u, F.transpose())) + Q

    # Update Step
    y_k = z_k - np.dot(H, x_k_p)
    S_k = np.dot(H, np.dot(P_k_p, H.transpose())) + R
    K_k = np.dot(P_k_p, np.dot(H, np.linalg.inv(S_k)))
    x_k_u = x_k_p + np.dot(K_k, y_k)
    P_k_u = np.dot((np.identity(3) - np.dot(K_k, H)), P_k_p)
